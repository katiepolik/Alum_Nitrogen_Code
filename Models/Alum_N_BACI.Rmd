...
PrePost_Depth <- left_join(PrePost_WS, LakeList, by = "DOW")
PrePost_Depth$Depth.Cat <- factor(PrePost_WS$Depth.Cat, levels = c("Shallow","Deep"))
...
ggplot(PrePost_Depth, aes(x = TP_perc, y = TN_perc, color = Depth.Cat, shape = Depth.Cat)) +
  ...
  geom_point(data = PrePost_Depth %>% filter(DOWApp == "62000600a"), color = "black", shape = 1, cex = 3.5) +
  geom_point(data = PrePost_Depth %>% filter(DOWApp == "62005500a"), color = "#DC3220", shape = 1, cex = 3.5) +
...
Absolute change models and plots (not included in publication)

```{r, fig.width=3, fig.height=3}
#percent change models and plots

TP.Litt.a <- lm(data = PrePost_Depth, TN_del ~ TP_del*Depth.Cat)
S.Litt.a <- lm(data = PrePost_Depth, TN_del ~ S_del*Depth.Cat)
TP.S.Litt.a <- lm(data = PrePost_Depth, S_del ~ TP_del*Depth.Cat)
tidy(TP.Litt.a)
tidy(S.Litt.a)
tidy(TP.S.Litt.a)

test(emtrends(TP.Litt.a, "Depth.Cat", var = "TP_del"))
test(emtrends(S.Litt.a, "Depth.Cat", var = "S_del"))
test(emtrends(TP.S.Litt.a, "Depth.Cat", var = "TP_del"))


ggplot(PrePost_Depth, aes(x = TP_del, y = TN_del, color = Depth.Cat, shape = Depth.Cat)) +
  scale_color_manual(values = c("#00BA38", "#619CFF")) +
  scale_shape_manual(values = c(17,19)) +
  geom_hline(yintercept = 0, lty = "dashed") +
  geom_vline(xintercept = 0, lty = "dashed") +
  ...
  ylab("Absolute Change in TN")
#ggsave("Litt_TN_TP.pdf", path = here("Plots"))

ggplot(PrePost_Depth, aes(x = S_del, y = TN_del, color = Depth.Cat, shape = Depth.Cat)) +
  scale_color_manual(values = c("#00BA38", "#619CFF")) +
  scale_shape_manual(values = c(17,19)) +
  geom_hline(yintercept = 0, lty = "dashed") +
  geom_vline(xintercept = 0, lty = "dashed") +
...
ggplot(PrePost_Depth, aes(y = S_del, x = TP_del, color = Depth.Cat, shape = Depth.Cat)) +
  scale_color_manual(values = c("#00BA38", "#619CFF")) +
  scale_shape_manual(values = c(17,19)) +
  geom_hline(yintercept = 0, lty = "dashed") +
  geom_vline(xintercept = 0, lty = "dashed") +
...
ggplot(PrePost_Depth) + 
  ...
  geom_point(data = PrePost_Depth %>% filter(Depth.Cat == T), aes(x = TP.pre.mol, y = TN.pre.mol), 
             color = "#619CFF", shape = 21, show.legend = F) +
  geom_point(data = PrePost_Depth %>% filter(Depth.Cat == F), aes(x = TP.pre.mol, y = TN.pre.mol), 
             color = "#00BA38", shape = 2, show.legend = F) +
  geom_point(data = PrePost_Depth %>% filter(Depth.Cat == T), aes(x = TP.post.mol, y = TN.post.mol), 
             color = "#619CFF", shape = 19, show.legend = F) +
  geom_point(data = PrePost_Depth %>% filter(Depth.Cat == F), aes(x = TP.post.mol, y = TN.post.mol), 
