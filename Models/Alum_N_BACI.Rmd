...
library(nlme) #for models with autocorrlation structure
...
#EDI: https://doi.org/10.6073/pasta/6ca2613b9bd553ff5cee1441d7c83452
...
AlumData_long = do.call(rbind, AlumList) %>% #make some changes for specific data or treatment date issues
  ...
  filter(!(DOW == "10000200" & sampleDate == "2020-06-03")) #alum treatment happened on 11-Jun-2020
...
#how much is < 0.03 or < 0.04, the most common detection limits
...
PrePostData <- PrePostData %>%
  ...
  mutate(DecimalDate = decimal_date(sampleDate),
         TimeOfYear = DecimalDate %% 1)
...
TP_BACI_nlme <- lme(log(TP.mg_L*1000) ~ treatment*evertreat2, 
                  data = PrePostData %>% filter(!is.na(TP.mg_L)), 
                  random = ~ 1 | DOWApp,
                  correlation = corCAR1(form = ~ DecimalDate | DOWApp))

#check_model(TP_BACI_nlme)
#acf(resid(TP_BACI_nlme, type = "normalized"))
#summary(TP_BACI_nlme)

TP.emm <- confint(emmeans(TP_BACI_nlme, ~ evertreat2*treatment, type = 'response'))

diffofdiff_TP <- (TP.emm$response[3] -  TP.emm$response[1]) - (TP.emm$response[4] -  TP.emm$response[2])
perc_TP <- exp(fixef(TP_BACI_nlme)[2]) - 1
perc_BACI_TP <- exp(fixef(TP_BACI_nlme)[4]*-1) - 1
...
TN_BACI_nlme <- lme(log(N.mg_L) ~ treatment*evertreat2, 
                  data = PrePostData %>% filter(!is.na(N.mg_L)), 
                  random = ~ 1 | DOWApp,
                  correlation = corCAR1(form = ~ DecimalDate | DOWApp))

#check_model(TN_BACI_nlme)
#acf(resid(TN_BACI_nlme, type = "normalized"))
#summary(TN_BACI_nlme)

N.emm <- confint(emmeans(TN_BACI_nlme, ~ evertreat2*treatment, type = 'response'))

diffofdiff_TN <- (N.emm$response[3] -  N.emm$response[1]) - (N.emm$response[4] -  N.emm$response[2])
perc_TN <- exp(fixef(TN_BACI_nlme)[2]) - 1
perc_BACI_TN <- exp(fixef(TN_BACI_nlme)[4]*-1) - 1
...
CHLA_BACI_nlme <- lme(log(chla.ug_L) ~ treatment*evertreat2, 
                  data = PrePostData %>% filter(!is.na(chla.ug_L)), 
                  random = ~ 1 | DOWApp,
                  correlation = corCAR1(form = ~ DecimalDate | DOWApp))

#check_model(CHLA_BACI_nlme)
#acf(resid(CHLA_BACI_nlme, type = "normalized"))

chla.emm <- confint(emmeans(CHLA_BACI_nlme, ~ evertreat2*treatment, type = 'response'))

diffofdiff_chla <- (chla.emm$response[3] -  chla.emm$response[1]) - (chla.emm$response[4] -  chla.emm$response[2])
perc_chla <- exp(fixef(CHLA_BACI_nlme)[2]) - 1
perc_BACI_chla <- exp(fixef(CHLA_BACI_nlme)[4]*-1) - 1
...
Secchi_BACI_nlme <- lme(log(secchi.m) ~ treatment*evertreat2, 
                  data = PrePostData %>% filter(!is.na(secchi.m)), 
                  random = ~ 1 | DOWApp,
                  correlation = corCAR1(form = ~ DecimalDate | DOWApp))

#check_model(Secchi_BACI_nlme)
#acf(resid(Secchi_BACI_nlme, type = "normalized"))

secchi.emm <- confint(emmeans(Secchi_BACI_nlme, ~ evertreat2*treatment, type = 'response'))

diffofdiff_secchi <- (secchi.emm$response[3] -  secchi.emm$response[1]) - (secchi.emm$response[4] -  secchi.emm$response[2])
perc_secchi <- exp(fixef(Secchi_BACI_nlme)[2]) - 1
perc_BACI_secchi <- exp(fixef(Secchi_BACI_nlme)[4]*-1) - 1


dodge <- position_dodge(width=0.25)
ggplot(secchi.emm, aes(x=factor(treatment), y=response, 
                       color=factor(evertreat2), 
                       group = factor(evertreat2),
                       shape = factor(evertreat2))) + 
  geom_line(show.legend = F, position = dodge, linetype = "dotted") +
...
NP_BACI_nlme <- lme(log(NP.ratio) ~ treatment*evertreat2, 
                  data = PrePostData %>% filter(!is.na(NP.ratio)), 
                  random = ~ 1 | DOWApp,
                  correlation = corCAR1(form = ~ DecimalDate | DOWApp))

#check_model(NP_BACI_nlme)
#acf(resid(NP_BACI_nlme, type = "normalized"))

np.emm <- confint(emmeans(NP_BACI_nlme, ~ evertreat2*treatment, type = 'response'))

diffofdiff_NP <- (np.emm$response[3] -  np.emm$response[1]) - (np.emm$response[4] -  np.emm$response[2])
perc_NP <- exp(fixef(NP_BACI_nlme)[2]) - 1
perc_BACI_NP <- exp(fixef(NP_BACI_nlme)[4]*-1) - 1
...
ggplot(np.emm, aes(x=factor(treatment), y=response, 
  ...
  xlab("") +
  expand_limits(y = 0) +
  scale_y_continuous(expand = c(0,0), limits = c(35,70))+
  scale_x_discrete(labels=c('Before', 'After'))
...
LongData2 <- LongData %>%
  ...
  mutate(plotYear = case_when(preLong == 1 ~ preYear - 9,
                             postLong == 1 ~ postYear)) %>%
  mutate(DOWApp2 = paste0(DOWApp,preLong),
         DecimalDate = decimal_date(sampleDate),
         TimeOfYear = DecimalDate %% 1,
         modDate = modYear + TimeOfYear)

```

```{r}
### TP model
P_nlme <- lme(log(TP.mg_L*1000) ~ modYear*preLong, 
                  data = LongData2 %>% filter(!is.na(TP.mg_L)), 
                  random = ~ 1 | DOWApp2,
                  correlation = corCAR1(form = ~ modDate | DOWApp2))

#acf(resid(P_nlme, type = "normalized"))
#check_model(P_nlme)
#summary(P_nlme)
#r2(P_nlme)
...
# Model that doesn't account for autocorrelation. Only for reference.
#N_lt_model2 <- lmer(data = LongData2 %>% filter(!is.na(N.mg_L)), 
#                      log(N.mg_L) ~ modYear*preLong + (1|DOWApp))

#Plots comparing w/ and w/o corCAR term
#pdf("acf_pre_CAR.pdf", width = 7, height = 5)
#acf(resid(N_lt_model2), main = "Without corCAR term")
#dev.off()

#pdf("acf_post_CAR.pdf", width = 7, height = 5)
#acf(resid(N_nlme, type = "normalized"), main = "With corCAR term")
#dev.off()

N_nlme <- lme(log(N.mg_L) ~ modYear*preLong, 
                  data = LongData2 %>% filter(!is.na(N.mg_L)), 
                  random = ~ 1 | DOWApp2,
                  correlation = corCAR1(form = ~ modDate | DOWApp2))

#acf(resid(N_nlme, type = "normalized"))
#check_model(N_nlme)
#summary(N_nlme)
#r2(N_nlme)
...
Secchi_nlme <- lme(log(secchi.m) ~ modYear*preLong, 
                  data = LongData2 %>% filter(!is.na(secchi.m)), 
                  random = ~ 1 | DOWApp2,
                  correlation = corCAR1(form = ~ modDate | DOWApp2))

#acf(resid(Secchi_nlme, type = "normalized"))
#check_model(Secchi_nlme)
#summary(Secchi_nlme)
#r2(Secchi_nlme)

### Chla model

Chla_nlme <- lme(log(chla.ug_L) ~ modYear*preLong, 
                  data = LongData2 %>% filter(!is.na(chla.ug_L)), 
                  random = ~ 1 | DOWApp2,
                  correlation = corCAR1(form = ~ modDate | DOWApp2))

#acf(resid(Chla_nlme, type = "normalized"))
#check_model(Chla_nlme)
#summary(Chla_nlme)
#r2(Chla_nlme)

### NP model

NP_nlme <- lme(log(NP.ratio) ~ modYear*preLong, 
                  data = LongData2 %>% filter(!is.na(NP.ratio)), 
                  random = ~ 1 | DOWApp2,
                  correlation = corCAR1(form = ~ modDate | DOWApp2))

#acf(resid(NP_nlme, type = "normalized"))
#check_model(NP_nlme)
#summary(NP_nlme)
#r2(NP_nlme)

#inspect these for significant slopes for pre and post
emt.TP <- test(emtrends(P_nlme, "preLong", var = "modYear"))
TP.ltemm <- confint(emmeans(P_nlme, ~ modYear*preLong, at = list(modYear = c(1,2,3,4,5,6,7,8)), 
                            type = 'response')) %>%
                    mutate(plotYear = case_when(preLong == 1 ~ modYear - 9, 
                                                TRUE ~ modYear))
LT.diff.TP <- mean(diff(TP.ltemm$response, lag = 1, differences = 1)[1:7])
LT.perc.TP <- exp(fixef(P_nlme)[2]) - 1
#
emt.TN <- test(emtrends(N_nlme, "preLong", var = "modYear"))
TN.ltemm <- confint(emmeans(N_nlme, ~ modYear*preLong, at = list(modYear = c(1,2,3,4,5,6,7,8)), 
                            type = 'response')) %>%
                    mutate(plotYear = case_when(preLong == 1 ~ modYear - 9, 
                                                TRUE ~ modYear))
LT.diff.TN <- mean(diff(TN.ltemm$response, lag = 1, differences = 1)[1:7])
LT.perc.TN <- exp(fixef(N_nlme)[2]) - 1
#
emt.NP <- test(emtrends(NP_nlme, "preLong", var = "modYear"))
NP.ltemm <- confint(emmeans(NP_nlme, ~ modYear*preLong, at = list(modYear = c(1,2,3,4,5,6,7,8)), 
                            type = 'response')) %>%
                    mutate(plotYear = case_when(preLong == 1 ~ modYear - 9, 
                                                TRUE ~ modYear))
LT.diff.NP <- mean(diff(NP.ltemm$response, lag = 1, differences = 1)[1:7])
LT.perc.NP <- exp(fixef(NP_nlme)[2]) - 1
#
emt.chla <- test(emtrends(Chla_nlme, "preLong", var = "modYear"))
chla.ltemm <- confint(emmeans(Chla_nlme, ~ modYear*preLong, at = list(modYear = c(1,2,3,4,5,6,7,8)), 
                            type = 'response')) %>%
                    mutate(plotYear = case_when(preLong == 1 ~ modYear - 9, 
                                                TRUE ~ modYear))
LT.diff.chla <- mean(diff(chla.ltemm$response, lag = 1, differences = 1)[1:7])
LT.perc.chla <- exp(fixef(Chla_nlme)[2]) - 1
#
emt.Secchi <- test(emtrends(Secchi_nlme, "preLong", var = "modYear"))
secchi.ltemm <- confint(emmeans(Secchi_nlme, ~ modYear*preLong, at = list(modYear = c(1,2,3,4,5,6,7,8)), 
                            type = 'response')) %>%
                    mutate(plotYear = case_when(preLong == 1 ~ modYear - 9, 
                                                TRUE ~ modYear))
LT.diff.secchi <- mean(diff(secchi.ltemm$response, lag = 1, differences = 1)[1:7])
LT.perc.secchi <- exp(fixef(Secchi_nlme)[2]) - 1
...
ggplot() + 
  ...
  geom_line(data = TN.ltemm, 
            aes(group = preLong, x = as.factor(plotYear), y = response, 
                colour = as.factor(preLong)),
            show.legend = F,
            lwd = 1.2) +
  scale_color_manual(values=c(I("black"), I("black"))) +
  ylab("Total Nitrogen (mg/L)") +
...
ggplot() + 
  ...
  geom_boxplot(width = 0.7, show.legend = F, data = LongData2 %>% filter(!is.na(TP.mg_L)), 
               aes(x=factor(plotYear), y=TP.mg_L*1000, fill=factor(preLong)),
               outlier.size=0.5, lwd = 0.5) +
  scale_fill_manual(values=c("grey", "white")) +
  geom_line(data = TP.ltemm, 
            aes(group = preLong, x = as.factor(plotYear), y = response, 
                colour = as.factor(preLong)),
            show.legend = F,
            lwd = 1.2) +
...
```
Stats for DeN assays
```{r}
DeN <- read.csv(here("Data/DeN_Rates.csv"))

DeN %>% group_by(Treat) %>% summarise(avg = mean(DeN), sd = sd(DeN))
t.test(data = DeN, DeN ~ Treat)
